...
(async () => {
    ...
    for (let i = 0; i < votesPerWorker; i++) {
        console.log(`Worker ${workerIndex} - Starting voting process ${i + 1} of ${votesPerWorker}`);
        ...
        const submit_btn = await page.waitForSelector("::-p-xpath(/html/body/div[6]/div[3]/div/section/div/div/div/button[1])");
        ...
        await page.screenshot({
            path: `screenshots/${mail}.jpg`
          });
        await browser.close();
        console.log(`Worker ${workerIndex} - Voting process ${i + 1} done on ${mail}`);
